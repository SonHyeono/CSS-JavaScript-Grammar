...
// 1. concat(), concatenation(집합, 연결) : 두개 이상의 배열을 합쳐서 새로운 배열을 반환
...
const fishString = fish.join(); //join() 내부에 별도의 파라미터를 정하지 않으면 ,를 기준으로 합쳐짐
...
// 4. indexOf() : 배열에서 찾고자 하는 요소의 가장 첫 번째 인덱스 위치만 반환 (찾고자하는 요소가 중복이어도)
const fish2 = ['shark', 'whale', 'koi', 'whale', 'nemo'];

console.log(fish2);
console.log(`fish array에서 'whale'의 가장 첫번째 위치 : ${fish2.indexOf('whale')}`);

// 찾고자 하는 요소가 없을 경우
console.log(`한치가 있나요? 한치는 오징어인데.... ${fish2.indexOf('hanchi')}`);
console.groupdEnd;
